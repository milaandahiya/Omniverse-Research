...
try:
    ...
    config.enable_stream(rs.stream.color, rs.format.rgb8, 60)

    # Start streaming from file
    pipeline.start(config)

    # get device serial number
    serial = pipeline.get_active_profile().get_device().get_info(rs.camera_info.serial_number)
    ...
    # get the realsense camera intrinsics and print matrix
    ...
    while True:
        ...
        cv2.imshow("Color / Depth Stream", np.concatenate((color_image, depth_color_image), axis=1))
        key = cv2.waitKey(1)
